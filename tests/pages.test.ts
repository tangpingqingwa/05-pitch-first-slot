...
test("occupied raise cue tells a founder who is not #1 what Polar charges", async () => {
  ...
  assert.equal((listingCard(html, "Stage Co").match(/class="open-deck"/g) ?? []).length, 1);
  assert.equal((listingCard(html, "Helix Labs").match(/class="open-deck"/g) ?? []).length, 1);
...
test("occupied paid cue names Open deck as the only outbound hop", async () => {
  ...
  assert.equal((paid.match(/href="\/listings\/[^"]+\/clicks"/g) ?? []).length, 1);
  assert.match(
    paid,
    new RegExp(
...
test("occupied #1 cue hops Then Outbid after Open deck", async () => {
  ...
  assert.match(paid, /class="raise-after-deck"/);
  ...
  assert.equal((paid.match(/href="#claim"/g) ?? []).length, 1);
  assert.equal((html.match(/data-raise-after-deck="true"/g) ?? []).length, 1);
  assert.equal((html.match(/Then Outbid/g) ?? []).length, 1);
  const hopAt = paid.indexOf('data-open-deck="true"');
