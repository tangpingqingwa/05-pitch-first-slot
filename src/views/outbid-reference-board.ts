import {
  ...
  withRanks,
  type RankedListing,
} from "./outbid-reference-core.js";
import { formatIssueDate } from "./outbid-reference-day.js";
import {
  displayHostPath,
  escapeHtml,
  formatUsd,
  html,
  relativeTime,
} from "./outbid-reference-html.js";
import { renderLayout, SITE_TITLE } from "./outbid-reference-layout.js";
...
function renderCategoryPicker(): string {
  return html`<div class="category-picker" data-category-picker="">
    ...
    <button type="button" class="category-select" id="category-select" role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-controls="category-menu" data-category-select="" data-slot="category-control">
      <span data-category-label>Choose a category</span>
    </button>
    <div class="category-menu" id="category-menu" role="listbox" aria-label="Choose a category" hidden data-category-menu="">
...
<script>
  (function () {
    ...
    function ready() {
      if (!submit) return;
      var isReady = Boolean(urlField && validUrl(urlField.value.trim()) && categoryValue && categoryValue.value && whyField && whyField.value.trim().length >= 8 && input && parseBid(input.value) >= min);
