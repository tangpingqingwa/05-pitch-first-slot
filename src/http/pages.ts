...
function deckHop(
  ...
  if (paid && openOne) {
    const next = raiseAfter
      ...
      : "";
    return `<p class="deck">
        <a class="open-deck open-one open-after-raise-one" data-open-deck="true" data-open-one="true" data-open-after-raise-one="true" href="${href}" rel="noopener noreferrer">
          Open deck
          <span class="deck-url">${url}</span>
        </a>${next}
      </p>`;
...
function renderCueCard(input: {
  ...
  const attrs = later
    ...
    : openOne
      ? `${input.attrs} data-open-one-first="true" data-raise-one-first="true" data-open-after-raise-one-first="true" data-raise-after-open-two-first="true"`
      : input.attrs;
  const body = later
