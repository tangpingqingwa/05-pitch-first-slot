...
if [[ -f package.json ]]; then
  ...
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-room' \
    || fail "occupied #1 room line must stamp quiet so ± Outbid stay the action"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'only the difference' \
    || fail "quiet room line must keep Polar raise-pays-difference"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-window' \
    || fail "quiet room line must not restamp occupied-week-window-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-charge' \
    || fail "quiet room line must not restamp occupied-raise-charge-quiet copy"
  grep -F -q '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts \
    || fail "occupied #1 room line must be quieted in occupied CSS"
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'font-size: 0.75rem' \
    || fail "occupied #1 room line must stay smaller than dashed \$amount"
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'var(--sans)' \
    || fail "occupied #1 room line must not lecture in serif over ± Outbid"
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'rgb(143, 122, 98)' \
    || fail "occupied #1 room line must stay muted so Outbid stays the action"
  if grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'var(--serif)'; then
    fail "occupied #1 room line must not sit as a serif lecture between ± and Outbid"
  fi
  if grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'font-size: 1.15rem'; then
    fail "occupied #1 room line must not match cream-serif lecture size over the dashed amount"
  fi
  if grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .room[data-quiet-room]' src/views/skin.ts | grep -q 'var(--cream)'; then
    fail "occupied #1 room line must not cream-lecture over ± Outbid"
  fi
  grep -A 8 '.claim-note .raise-charge' src/views/skin.ts | grep -q 'font-size: 0.75rem' \
    || fail "quiet room line must keep occupied raise-charge muted"
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-note .week-window[data-rolling-week]' src/views/skin.ts | grep -q 'font-size: 0.75rem' \
    || fail "quiet room line must keep occupied rolling-week cue muted"
  if awk '/^export const HOUSE_CSS/{p=1} p{print} /^export const OCCUPIED_CSS/{exit}' src/views/skin.ts \
    | grep -Eq 'data-quiet-room'; then
    fail "HOUSE_CSS must not stamp occupied quiet room chrome"
  fi
  if grep -n 'function raiseAfterDeckHop' -A 12 src/http/pages.ts | grep -Eq 'quiet-room|data-quiet-room|#1 is \$'; then
    fail "must not put Polar lecture on the #1 cue"
  fi
  if grep -n 'data-empty-room' -A 8 src/http/pages.ts | grep -Eq 'data-quiet-room'; then
    fail "empty Claim-first must not stamp occupied quiet room"
  fi
  ...
  grep -q 'class="bid-row"' src/http/pages.ts \
    || fail "quiet room line cut must keep occupied bid-row"
  grep -q 'data-first-click="claim"' src/http/pages.ts \
    || fail "quiet room line cut must keep empty Claim / Outbid as the first click"
  grep -q 'class="bid-form later-write" data-later-write="true"' src/http/pages.ts \
    || fail "quiet room line cut must keep empty deck URL as a later write"
  grep -q 'data-bid-step' src/http/pages.ts \
    || fail "quiet room line cut must keep ±"
  grep -q 'class="outbid">Outbid' src/http/pages.ts \
    || fail "quiet room line cut must keep Outbid"
  grep -q 'bid-field' src/http/pages.ts \
    || fail "quiet room line cut must keep the dashed amount"
  grep -q 'Polar charged the difference' src/http/pages.ts \
    || fail "quiet room line cut must not restamp checkout-raise-copy"
  grep -q 'Sunday pay raised Monday still pays the difference' src/http/pages.ts \
    || fail "Sunday→Monday raise-pays-difference must stay on checkout return"
  grep -q 'Same listing still inside last 7 days' src/http/pages.ts \
    || fail "quiet room line cut must not restamp raise-rolling-identity"
  grep -q 'occupied claim keeps raise-pays-difference short' tests/pages.test.ts \
    || fail "quiet room line cut must not restamp occupied-claim-short copy"
  grep -q 'occupied raise-charge stays quiet' tests/pages.test.ts \
    || fail "quiet room line cut must not restamp occupied-raise-charge-quiet copy"
  grep -q 'occupied rolling-week cue stays quiet' tests/pages.test.ts \
    || fail "quiet room line cut must not restamp occupied-week-window-quiet copy"
  if grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'The $ you type is the public bid'; then
    fail "quiet room line must not restamp occupied-claim-short lecture"
  fi
  if grep -Eq 'raise-after-open-seven|open-after-raise-six' src/http/pages.ts src/views/skin.ts; then
    fail "quiet room line cut must not add another named hop"
  fi

  echo "== UX: occupied claim-note sits after Outbid — ± Outbid stay adjacent =="
  grep -q 'occupied claim-note sits after Outbid' tests/pages.test.ts \
    || fail "pages tests must cover occupied claim-note after Outbid"
  grep -q 'claim-note sits after Outbid' "$test_log" \
    || fail "pages tests must run occupied claim-note after Outbid"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-after-outbid' \
    || fail "occupied claim-note must stamp after-outbid so ± Outbid stay adjacent"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'only the difference' \
    || fail "claim-note after Outbid must keep Polar raise-pays-difference"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-room' \
    || fail "claim-note after Outbid must not restamp occupied-room-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-window' \
    || fail "claim-note after Outbid must not restamp occupied-week-window-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-charge' \
    || fail "claim-note after Outbid must not restamp occupied-raise-charge-quiet copy"
  grep -q 'occupiedNoteAfterOutbid' src/http/pages.ts \
    || fail "occupied claim-note must compose after Outbid, not between ± and Outbid"
  ...
  awk '
    /: `<form id="bid-form" class="bid-form" method/ {p=1}
    p && /class="outbid">Outbid/ && !out {out=NR}
    p && /occupiedNoteAfterOutbid \? note/ {note=NR}
    p && /<\/form>`/ {exit}
    END { if (!(out && note && note > out)) exit 1 }
  ' src/http/pages.ts \
    || fail "occupied form must put the claim-note after the Outbid button"
  grep -F -q '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-form' src/views/skin.ts \
    || fail "occupied bid-form must sit closer to ± so Outbid stays adjacent"
  grep -F -q '.house-occupied[data-occupied-house] .claim-note[data-after-outbid]' src/views/skin.ts \
    || fail "occupied claim-note after Outbid must be composed in occupied CSS"
  grep -F -A 4 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-form' src/views/skin.ts | grep -q 'margin-top: 0.5rem' \
    || fail "occupied bid-form must tighten to ± so Outbid stays adjacent"
  grep -F -A 4 '.house-occupied[data-occupied-house] .claim-note[data-after-outbid]' src/views/skin.ts | grep -q 'margin-top: 0.7rem' \
    || fail "occupied claim-note after Outbid must recede below the action"
  if awk '/^export const HOUSE_CSS/{p=1} p{print} /^export const OCCUPIED_CSS/{exit}' src/views/skin.ts \
    | grep -Eq 'data-after-outbid'; then
    fail "HOUSE_CSS must not stamp occupied claim-note-after-outbid chrome"
  fi
  if grep -n 'function raiseAfterDeckHop' -A 12 src/http/pages.ts | grep -Eq 'after-outbid|data-after-outbid'; then
    fail "must not put Polar lecture on the #1 cue"
  fi
  if grep -n 'data-empty-room' -A 8 src/http/pages.ts | grep -Eq 'data-after-outbid'; then
    fail "empty Claim-first must not stamp occupied claim-note after Outbid"
  fi
  grep -n 'data-empty-room' -A 4 src/http/pages.ts | grep -q 'The room is empty' \
    || fail "empty house must keep The room is empty before Claim / Outbid"
  grep -q 'class="bid-row"' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep occupied bid-row"
  grep -q 'data-first-click="claim"' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep empty Claim / Outbid as the first click"
  grep -q 'class="bid-form later-write" data-later-write="true"' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep empty deck URL as a later write"
  grep -q 'data-bid-step' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep ±"
  grep -q 'class="outbid">Outbid' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep Outbid"
  grep -q 'bid-field' src/http/pages.ts \
    || fail "claim-note after Outbid cut must keep the dashed amount"
  grep -q 'Polar charged the difference' src/http/pages.ts \
    || fail "claim-note after Outbid cut must not restamp checkout-raise-copy"
  grep -q 'Sunday pay raised Monday still pays the difference' src/http/pages.ts \
    || fail "Sunday→Monday raise-pays-difference must stay on checkout return"
  grep -q 'Same listing still inside last 7 days' src/http/pages.ts \
    || fail "claim-note after Outbid cut must not restamp raise-rolling-identity"
  grep -q 'occupied claim keeps raise-pays-difference short' tests/pages.test.ts \
    || fail "claim-note after Outbid cut must not restamp occupied-claim-short copy"
  grep -q 'occupied raise-charge stays quiet' tests/pages.test.ts \
    || fail "claim-note after Outbid cut must not restamp occupied-raise-charge-quiet copy"
  grep -q 'occupied rolling-week cue stays quiet' tests/pages.test.ts \
    || fail "claim-note after Outbid cut must not restamp occupied-week-window-quiet copy"
  grep -q 'occupied #1 room line stays quiet' tests/pages.test.ts \
    || fail "claim-note after Outbid cut must not restamp occupied-room-quiet copy"
  if grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'The $ you type is the public bid'; then
    fail "claim-note after Outbid must not restamp occupied-claim-short lecture"
  fi
  if grep -Eq 'raise-after-open-seven|open-after-raise-six' src/http/pages.ts src/views/skin.ts; then
  ...
  echo "== UX: occupied one-liner recedes with company/url so ± Outbid stay the action cluster =="
  grep -q 'occupied one-liner recedes with company/url' tests/pages.test.ts \
    || fail "pages tests must cover occupied one-liner recede"
  grep -q '± Outbid stay the action cluster' "$test_log" \
    || fail "pages tests must run occupied one-liner recede"
  grep -q 'data-oneliner="true"' src/http/pages.ts \
    || fail "occupied one-liner must recede with company/url in the after-action bid-row"
  grep -n 'occupiedOutbidBesidePlus' -A 8 src/http/pages.ts | grep -q 'data-oneliner="true"' \
    || fail "occupied one-liner must sit in the receded bid-row, not as a later full-weight field"
  grep -q 'data-beside-plus="true"' src/http/pages.ts \
    || fail "one-liner recede must keep occupied Outbid beside ±"
  grep -q 'data-after-action="true"' src/http/pages.ts \
    || fail "one-liner recede must keep company/url receding after Outbid"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'only the difference' \
    || fail "one-liner recede must keep Polar raise-pays-difference"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-after-outbid' \
    || fail "one-liner recede must not restamp occupied-claim-note-after-outbid copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-room' \
    || fail "one-liner recede must not restamp occupied-room-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-window' \
    || fail "one-liner recede must not restamp occupied-week-window-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-charge' \
    || fail "one-liner recede must not restamp occupied-raise-charge-quiet copy"
  grep -q '${occupiedNoteAfterOutbid ? "" : note}' src/http/pages.ts \
    || fail "empty house must keep the claim-note before Claim / Outbid"
  grep -q '${occupiedNoteAfterOutbid ? note : ""}' src/http/pages.ts \
    || fail "occupied form must interpolate the claim-note after Outbid"
  awk '
    /data-after-action="true"/ {row=NR}
    /data-oneliner="true"/ {one=NR}
    /occupiedNoteAfterOutbid \? note/ {note=NR}
    END { if (!(row && one && note && one > row && note > one)) exit 1 }
  ' src/http/pages.ts \
    || fail "occupied template must recede one-liner with company/url before the quiet claim-note"
  grep -F -q '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-row[data-after-action] .field[data-oneliner]' src/views/skin.ts \
    || fail "occupied one-liner must recede with company/url in occupied CSS"
  grep -F -A 4 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-row[data-after-action] .field[data-oneliner]' src/views/skin.ts | grep -q 'flex: 1 1 100%' \
    || fail "occupied one-liner must recede under company/url, not as a full-weight field"
  grep -F -A 4 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-row[data-after-action] .field input' src/views/skin.ts | grep -q 'font-size: 0.88rem' \
    || fail "occupied one-liner must recede at the same quiet size as company/url"
  grep -F -A 6 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .claim .outbid[data-beside-plus]' src/views/skin.ts | grep -q 'font-weight: 700' \
    || fail "one-liner recede must keep occupied Outbid the action in the cluster"
  grep -F -A 6 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-row[data-after-action]' src/views/skin.ts | grep -q 'border-top: 1px dashed' \
    || fail "one-liner recede must keep the dashed separator after Outbid"
  if awk '/^export const HOUSE_CSS/{p=1} p{print} /^export const OCCUPIED_CSS/{exit}' src/views/skin.ts \
    | grep -Eq 'data-oneliner'; then
    fail "HOUSE_CSS must not stamp occupied one-liner recede chrome"
  fi
  if grep -n 'function raiseAfterDeckHop' -A 12 src/http/pages.ts | grep -Eq 'data-oneliner'; then
    fail "must not put Polar lecture on the #1 cue"
  fi
  if grep -n 'data-empty-room' -A 8 src/http/pages.ts | grep -Eq 'data-oneliner'; then
    fail "empty Claim-first must not stamp occupied one-liner recede"
  fi
  if grep -n 'function renderUnranked' -A 14 src/http/pages.ts | grep -Eq 'data-oneliner'; then
    fail "unpaid cue must not stamp occupied one-liner recede"
  fi
  if grep -n 'occupiedHouse === true' -A 8 src/http/pages.ts | grep -Eq 'later-write|data-first-click="claim"'; then
    fail "occupied / must not wrap Claim as empty later-write"
  fi
  grep -n 'data-empty-room' -A 4 src/http/pages.ts | grep -q 'The room is empty' \
    || fail "empty house must keep The room is empty before Claim / Outbid"
  grep -q 'class="bid-row"' src/http/pages.ts \
    || fail "one-liner recede cut must keep occupied bid-row DNA"
  grep -q 'data-first-click="claim"' src/http/pages.ts \
    || fail "one-liner recede cut must keep empty Claim / Outbid as the first click"
  grep -q 'class="bid-form later-write" data-later-write="true"' src/http/pages.ts \
    || fail "one-liner recede cut must keep empty deck URL as a later write"
  grep -q 'data-bid-step' src/http/pages.ts \
    || fail "one-liner recede cut must keep ±"
  grep -q 'class="outbid">Outbid' src/http/pages.ts \
    || fail "one-liner recede cut must keep Outbid"
  grep -q 'bid-field' src/http/pages.ts \
    || fail "one-liner recede cut must keep the dashed amount"
  grep -q 'Polar charged the difference' src/http/pages.ts \
    || fail "one-liner recede cut must not restamp checkout-raise-copy"
  grep -q 'Sunday pay raised Monday still pays the difference' src/http/pages.ts \
    || fail "Sunday→Monday raise-pays-difference must stay on checkout return"
  grep -q 'Same listing still inside last 7 days' src/http/pages.ts \
    || fail "one-liner recede cut must not restamp raise-rolling-identity"
  grep -q 'occupied claim keeps raise-pays-difference short' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-claim-short copy"
  grep -q 'occupied raise-charge stays quiet' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-raise-charge-quiet copy"
  grep -q 'occupied rolling-week cue stays quiet' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-week-window-quiet copy"
  grep -q 'occupied #1 room line stays quiet' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-room-quiet copy"
  grep -q 'occupied claim-note sits after Outbid' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-claim-note-after-outbid copy"
  grep -q 'occupied Outbid sits beside ±' tests/pages.test.ts \
    || fail "one-liner recede cut must not restamp occupied-outbid-beside-plus copy"
  if grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'The $ you type is the public bid'; then
    fail "one-liner recede must not restamp occupied-claim-short lecture"
  fi
  if grep -Eq 'raise-after-open-seven|open-after-raise-six' src/http/pages.ts src/views/skin.ts; then
    fail "one-liner recede cut must not add another named hop"
  fi

  echo "== UX: occupied claim-after-slot headline recedes so ± Outbid stay the action cluster =="
  grep -q 'occupied claim-after-slot headline recedes so ± Outbid stay the action cluster' tests/pages.test.ts \
    || fail "pages tests must cover occupied claim-after-slot headline recede"
  grep -q 'occupied claim-after-slot headline recedes' "$test_log" \
    || fail "pages tests must run occupied claim-after-slot headline recede"
  grep -q 'data-quiet-headline="true"' src/http/pages.ts \
    || fail "occupied claim-after-slot headline must stamp quiet-headline so it recedes"
  grep -n 'occupiedOutbidBesidePlus' -A 4 src/http/pages.ts | grep -q 'data-quiet-headline="true"' \
    || fail "occupied claim-after-slot headline must recede only when ± Outbid are the action cluster"
  grep -q 'Opening three minutes' src/http/pages.ts \
    || fail "occupied claim-after-slot headline must recede, not vanish"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'only the difference' \
    || fail "headline recede must keep Polar raise-pays-difference"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-after-outbid' \
    || fail "headline recede must not restamp occupied-claim-note-after-outbid copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-room' \
    || fail "headline recede must not restamp occupied-room-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-window' \
    || fail "headline recede must not restamp occupied-week-window-quiet copy"
  grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'data-quiet-charge' \
    || fail "headline recede must not restamp occupied-raise-charge-quiet copy"
  grep -q 'data-beside-plus="true"' src/http/pages.ts \
    || fail "headline recede must not restamp occupied-outbid-beside-plus"
  grep -q 'data-oneliner="true"' src/http/pages.ts \
    || fail "headline recede must not restamp occupied-oneliner-recede"
  grep -q '${occupiedNoteAfterOutbid ? "" : note}' src/http/pages.ts \
    || fail "empty house must keep the claim-note before Claim / Outbid"
  grep -q '${occupiedNoteAfterOutbid ? note : ""}' src/http/pages.ts \
    || fail "occupied form must interpolate the claim-note after Outbid"
  awk '
    /class="stage-head"/ {head=NR}
  ...
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .stage-head[data-quiet-headline] .headline' src/views/skin.ts | grep -q 'font-family: var(--sans)' \
    || fail "occupied claim-after-slot headline must recede off display serif"
  grep -F -A 8 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .stage-head[data-quiet-headline] .headline' src/views/skin.ts | grep -q 'font-size: 0.75rem' \
  ...
  grep -F -A 6 '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .claim .outbid[data-beside-plus]' src/views/skin.ts | grep -q 'font-weight: 700' \
    || fail "headline recede must keep occupied Outbid the action in the cluster"
  grep -F -q '.house-occupied[data-occupied-house] .claim-after-slot[data-claim-after-slot] .bid-row[data-after-action] .field[data-oneliner]' src/views/skin.ts \
    || fail "headline recede must keep occupied one-liner receded with company/url"
  if grep -q 'clamp(1.35rem, 4.2vw, 1.85rem)' src/views/skin.ts; then
    fail "occupied claim-after-slot headline must recede, not stay a full prize title"
  fi
  ...
  if awk '/^export const HOUSE_CSS/{p=1} p{print} /^export const OCCUPIED_CSS/{exit}' src/views/skin.ts \
    | grep -Eq 'data-quiet-headline'; then
    fail "HOUSE_CSS must not stamp occupied headline recede chrome"
  fi
  ...
  grep -n 'data-empty-room' -A 4 src/http/pages.ts | grep -q 'The room is empty' \
    || fail "empty house must keep The room is empty before Claim / Outbid"
  grep -q 'class="bid-row"' src/http/pages.ts \
    || fail "headline recede cut must keep occupied bid-row DNA"
  grep -q 'data-first-click="claim"' src/http/pages.ts \
    || fail "headline recede cut must keep empty Claim / Outbid as the first click"
  grep -q 'class="bid-form later-write" data-later-write="true"' src/http/pages.ts \
    || fail "headline recede cut must keep empty deck URL as a later write"
  grep -q 'data-bid-step' src/http/pages.ts \
    || fail "headline recede cut must keep ±"
  grep -q 'class="outbid">Outbid' src/http/pages.ts \
    || fail "headline recede cut must keep Outbid"
  grep -q 'bid-field' src/http/pages.ts \
    || fail "headline recede cut must keep the dashed amount"
  grep -q 'Polar charged the difference' src/http/pages.ts \
    || fail "headline recede cut must not restamp checkout-raise-copy"
  grep -q 'Sunday pay raised Monday still pays the difference' src/http/pages.ts \
    || fail "Sunday→Monday raise-pays-difference must stay on checkout return"
  grep -q 'Same listing still inside last 7 days' src/http/pages.ts \
    || fail "headline recede cut must not restamp raise-rolling-identity"
  grep -q 'occupied claim keeps raise-pays-difference short' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-claim-short copy"
  grep -q 'occupied raise-charge stays quiet' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-raise-charge-quiet copy"
  grep -q 'occupied rolling-week cue stays quiet' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-week-window-quiet copy"
  grep -q 'occupied #1 room line stays quiet' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-room-quiet copy"
  grep -q 'occupied claim-note sits after Outbid' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-claim-note-after-outbid copy"
  grep -q 'occupied Outbid sits beside ±' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-outbid-beside-plus copy"
  grep -q 'occupied one-liner recedes with company/url' tests/pages.test.ts \
    || fail "headline recede cut must not restamp occupied-oneliner-recede copy"
  if grep -n 'data-occupied-raise' -A 6 src/http/pages.ts | grep -q 'The $ you type is the public bid'; then
    fail "headline recede must not restamp occupied-claim-short lecture"
  fi
